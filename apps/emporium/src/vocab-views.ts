 ...
 *
 * MOVED here from apps/organism/src/cell/vocab-views.ts (U10, the emporium
 * migration, design §5) — apps/emporium is its only external consumer
 * (unanimous judge ruling); this keeps @shrubbery/source's browser-safe '.'
 * barrel lit-free (this view imports lit + side-effect-imports
 * @shrubbery/components, so it does NOT belong in the package charter — the
 * emporium-store/-client transport code moved to @shrubbery/source/emporium
 * instead, since that's squarely transport, not view). apps/organism keeps
 * its own byte-identical copy in place (frozen under active swarm ownership,
 * deprecated-in-place, zero external consumers after this migration).
 *
 ...
 * render package — here, alongside the dedicated Emporium app shell that is its
 * only consumer.
 *
...
// shell already imports '@shrubbery/components' for the chrome; importing the

/**
 * client.ts — SHELL-SIDE live read of a gardend cell's /emporium vocab
 ...
 *
 * HOISTED from apps/organism/src/cell/emporium-client.ts (U10, the emporium
 * migration) — @shrubbery/source/emporium is the sole external
 * adapter-consumption convention; organism keeps its own copy in place
 * (frozen under active swarm ownership) but it no longer has an external
 * consumer. Byte-identical transport logic, renamed to this package's './emporium'
 * subpath charter.
 *
 * The library (@shrubbery/render) is network-free and only knows how to RENDER
 * the VocabResource / VocabPackResource shapes; this client READS them live and
 * maps the upstream JSON into those shapes. Nothing is mocked: every read is a
 * real GET against a real cell.
 *

/**
 * emporium-store.ts — SHELL-SIDE reactive store over the live EMPORIUM vocab
 * catalogue, mirroring session-store.ts.
 *
 * It wraps the EmporiumClient (the live /emporium reader) in a tiny reactive
 * store the organism binds the DOM views to. The store:
 ...
 *
 * This is SHELL state. The pure views (vocab-views.ts) are handed finished
 * VocabSummary[] / VocabPack values; they never fetch.
 */

import type { VocabPack, VocabSummary } from '@shrubbery/render'
import { EmporiumClient, type EmporiumTransport } from './emporium-client.js'
